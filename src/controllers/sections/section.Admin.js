...
adminEdit.updateProfile = async (req, res) => {
  ...
  console.log(emailUserConcurrent);
  ...
  connectionDB.query("SELECT * FROM administrator WHERE email_center = ?", [emailUserConcurrent], async (err, rows) => {
    ...
    if (!err && rows.length > 0) {
      ...
      console.log("base de datos",imgAdmin);
...
adminEdit.getData = async (req, res) => {
  connectionDB.query("SELECT * FROM administrator WHERE email_center = ?", [req.user.emailUser], (err, rows) => {
    if (rows.length > 0) {
      return res.status(200).send({

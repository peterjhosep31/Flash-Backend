...
controllerStore.postStore = async (req, res) => {
  ...
  await connectionDB.query("SELECT password_admin FROM administrator WHERE email_admin = ?", [emailAdmin], async (err, rows) => {
    if (err) {
      ...
      if (comparePassword) {
        await connectionDB.query("SELECT * FROM store WHERE name_store = ?", [nameStore], async (err, rows) => {
          if (!err) {
            if (rows.length > 0) {
    ...
    async (err, rows) => {
      if (!err) {
        if (rows.length > 0) {
          ...
          let comparePassword = await bcryptjs.matchPassword(passwordEmployee, passwordEmployeeDB);
          if (comparePassword) {
            await connectionDB.query("SELECT * FROM store WHERE id_store = ?", [idStore], async (err, rows) => {
              if (rows.length != 0) {
                ...
                await connectionDB.query("UPDATE store SET ? WHERE id_store = ?", [
                  {
                    name_store: nameStore,
                    location_store: locationStore,
...
controllerStore.deleteStore = async (req, res) => {
  ...
  await connectionDB.query("SELECT password_admin FROM administrator WHERE email_admin = ?", [idUser], async (err, rows) => {
    if (rows.length == 0) {
      return res.status(404).send({
        mensaje: "El usuario no existe"
      });
      ...
      if (comparePassword) {
        connectionDB.query("SELECT name_store FROM store WHERE id_store = ?", [idStore], async (err, rows) => {
          if (rows.length == 0) {
            return res.status(404).send({
              mensaje: "El local no existe"
            });
